...
type Nodo = { tipo: 'admin' | 'sedes' | 'mapa' | 'personas' | 'recursos' | 'uc'; id?: string } | null;
type NodoGrafo = { key: string; tipo: 'admin' | 'sedes' | 'mapa' | 'personas' | 'recursos' | 'uc' | 'negocio'; id?: string; label: string; color: string };
...
export function VistaProyecto({ proyectoId, onVolver, volverLabel = '← Grafo del workspace' }: { proyectoId: string; onVolver: () => void; volverLabel?: string }) {
  ...
  const abrev = (t: NodoGrafo['tipo']) => t === 'uc' ? 'UC' : t === 'admin' ? '📄' : t === 'sedes' ? 'SED' : t === 'mapa' ? 'MAP' : t === 'personas' ? '👥' : t === 'recursos' ? '📦' : 'NEG';
  ...
  function abrirNodo(n: NodoGrafo) {
    ...
    if (n.tipo === 'admin' || n.tipo === 'sedes' || n.tipo === 'mapa' || n.tipo === 'personas' || n.tipo === 'recursos') setNodo({ tipo: n.tipo });
  }
      ...
      {!loading && (
        <div style={{ display: 'grid', gridTemplateColumns: movil ? '1fr' : 'minmax(280px, 4fr) 8fr', gap: '1rem', alignItems: 'start', marginTop: '0.75rem' }}>
          ...
          <div style={{ border: '1px solid #eee', borderRadius: 10, background: '#fcfcfc' }}>
            ...
            <p style={{ fontSize: 12, color: '#888', padding: '0 0.75rem 0.5rem' }}><span style={{ color: '#33415c' }}>📄 Planos = ver y descargar los documentos</span> · SED = Sedes & Espacios · <span style={{ color: '#b8860b' }}>MAP = Mapa Operativo</span> · <span style={{ color: '#8a4fbf' }}>👥 Personas & RH</span> · <span style={{ color: '#a9720f' }}>📦 Recursos & Proveedores</span> · <span style={{ color: '#8a4fbf' }}>NEG = Negocio</span> · UC = Unidad Comercial. Los nodos SED/MAP/👥/📦/UC <strong>alimentan</strong> los planos; en 📄 los ves. Clic para entrar.</p>
          </div>
        </div>
      )}
    </section>
