        ...
        <div style={{ display: 'grid', gridTemplateColumns: movil ? '1fr' : 'minmax(300px, 4fr) 8fr', gap: '1rem', alignItems: 'start', marginTop: '0.75rem' }}>
          <div style={{ border: '1px solid #cdd8ef', borderRadius: 10, padding: '0.75rem', background: 'var(--bp-panel-alt)' }}>
            ...
            <div style={{ marginTop: '0.6rem', borderTop: '1px solid #e3e9f5', paddingTop: '0.4rem' }}>
              {estados.map((e) => (
                <span key={e} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginRight: 8, fontSize: 11, color: 'var(--bp-muted)' }}>
...
function PanelSimulacion({ proyectoId, onVolver }: { proyectoId: string; onVolver: () => void }) {
  ...
  const stat: CSSProperties = { border: '1px solid #bcd8e6', borderRadius: 9, padding: '0.5rem 0.7rem', background: 'var(--bp-panel)', textAlign: 'center' };
  const v = rep ? VEREDICTO_INFO[rep.veredicto]! : null;

  return (
    <section>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
        <h2 style={{ margin: 0 }}>🏢 Simulación empresarial <span style={{ fontSize: 13, color: 'var(--bp-muted)' }}>· valida la lógica antes de publicar</span></h2>
        <button style={btn} onClick={onVolver}>← Planos</button>
      </div>
      <p style={{ fontSize: 12, color: 'var(--bp-muted)', margin: '0.3rem 0 0.6rem' }}>
      ...
      {/* Supuestos */}
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: '0.7rem', background: 'var(--bp-panel-alt)', border: '1px solid #cfe3ea', borderRadius: 9, padding: '0.5rem 0.7rem' }}>
        <label style={{ fontSize: 11.5, color: 'var(--bp-muted)' }}>Min/servicio<br /><input style={{ width: 80, padding: '0.25rem', borderRadius: 5, border: '1px solid var(--bp-border)' }} value={tServicio} onChange={(e) => setTServicio(e.target.value)} /></label>
        <label style={{ fontSize: 11.5, color: 'var(--bp-muted)' }}>Servicios/día<br /><input style={{ width: 90, padding: '0.25rem', borderRadius: 5, border: '1px solid var(--bp-border)' }} placeholder="máx" value={serviciosDia} onChange={(e) => setServiciosDia(e.target.value)} /></label>
        <label style={{ fontSize: 11.5, color: 'var(--bp-muted)' }}>Horas/día<br /><input style={{ width: 70, padding: '0.25rem', borderRadius: 5, border: '1px solid var(--bp-border)' }} value={horas} onChange={(e) => setHoras(e.target.value)} /></label>
        <label style={{ fontSize: 11.5, color: 'var(--bp-muted)' }}>Días/mes<br /><input style={{ width: 70, padding: '0.25rem', borderRadius: 5, border: '1px solid var(--bp-border)' }} value={dias} onChange={(e) => setDias(e.target.value)} /></label>
      ...
      {rep && v && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '0.5rem', marginBottom: '0.8rem' }}>
            <div style={stat}><div style={{ fontSize: 18, fontWeight: 'bold', color: '#2b7a93' }}>{rep.capacidad.serviciosPorDiaMax}</div><div style={{ fontSize: 11, color: 'var(--bp-muted)' }}>servicios/día (capacidad)</div></div>
            <div style={stat}><div style={{ fontSize: 18, fontWeight: 'bold', color: '#2b7a93' }}>{rep.produccion.serviciosPorMes}</div><div style={{ fontSize: 11, color: 'var(--bp-muted)' }}>servicios/mes (simulado)</div></div>
            <div style={stat}><div style={{ fontSize: 18, fontWeight: 'bold', color: 'var(--bp-text)' }}>{rep.capacidad.tiempoPorServicioMin} min</div><div style={{ fontSize: 11, color: 'var(--bp-muted)' }}>por servicio · {rep.capacidad.cabinas} cabina(s)</div></div>
            <div style={stat}><div style={{ fontSize: 18, fontWeight: 'bold', color: '#8a93a8' }}>{rep.capacidad.tiempoTotalMapaMin} min</div><div style={{ fontSize: 11, color: 'var(--bp-muted)' }}>carga total del mapa</div></div>
            <div style={stat}><div style={{ fontSize: 18, fontWeight: 'bold', color: '#8a4fbf' }}>{rep.nomina.personasInternas}</div><div style={{ fontSize: 11, color: 'var(--bp-muted)' }}>personas internas</div></div>
            <div style={stat}><div style={{ fontSize: 18, fontWeight: 'bold', color: rep.consumo.some((c) => c.alerta) ? '#c0392b' : '#2e9e63' }}>{rep.consumo.filter((c) => c.alerta).length}</div><div style={{ fontSize: 11, color: 'var(--bp-muted)' }}>insumos en alerta</div></div>
            <div style={stat}><div style={{ fontSize: 18, fontWeight: 'bold', color: rep.riesgos.proveedoresUnicosSinPlanB ? '#c0392b' : '#2e9e63' }}>{rep.riesgos.proveedoresUnicosSinPlanB}</div><div style={{ fontSize: 11, color: 'var(--bp-muted)' }}>proveedor único sin plan B</div></div>
          </div>

          {/* Consumo de insumos */}
          {rep.consumo.length > 0 && (
            <div style={{ border: '1px solid #e0d3b0', borderRadius: 9, padding: '0.6rem', marginBottom: '0.7rem', background: 'var(--bp-panel)' }}>
              <strong style={{ fontSize: 13, color: 'var(--bp-text)' }}>🧴 Consumo de insumos a {rep.produccion.serviciosPorDia} servicios/día</strong>
              <div style={{ overflowX: 'auto', marginTop: 4 }}>
                <table style={{ borderCollapse: 'collapse', fontSize: 12, width: '100%' }}>
                  <thead><tr>{['Insumo', 'Por servicio', 'Por mes', 'Stock', 'Cobertura', ''].map((h) => <th key={h} style={{ border: '1px solid var(--bp-border)', padding: '2px 6px', background: 'var(--bp-panel-alt)', textAlign: 'left' }}>{h}</th>)}</tr></thead>
                  <tbody>
                    {rep.consumo.map((c, i) => (
                      <tr key={i} style={{ background: c.alerta ? '#fdecea' : 'transparent' }}>
                        <td style={{ border: '1px solid var(--bp-border)', padding: '2px 6px' }}>{c.insumo}{!c.enCatalogo && <span style={{ color: '#c60' }}> ⚠ sin catálogo</span>}</td>
                        <td style={{ border: '1px solid var(--bp-border)', padding: '2px 6px' }}>{c.porServicio} {c.unidad}</td>
                        <td style={{ border: '1px solid var(--bp-border)', padding: '2px 6px' }}>{c.porMes} {c.unidad}</td>
                        <td style={{ border: '1px solid var(--bp-border)', padding: '2px 6px' }}>{c.stockActual ?? '—'}</td>
                        <td style={{ border: '1px solid var(--bp-border)', padding: '2px 6px', color: c.alerta ? '#c0392b' : '#555' }}>{c.diasCobertura !== null ? `${c.diasCobertura} d` : '—'}</td>
                        <td style={{ border: '1px solid var(--bp-border)', padding: '2px 6px' }}>{c.alerta ? '🔴' : '🟢'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: '0.5rem', marginBottom: '0.7rem' }}>
            <div style={{ border: '1px solid #e0d3b0', borderRadius: 9, padding: '0.5rem 0.7rem', background: 'var(--bp-panel)' }}>
              <strong style={{ fontSize: 12.5, color: 'var(--bp-text)' }}>💵 Ingresos (fuentes: {rep.ingresos.fuentes})</strong>
            ...
            <div style={{ border: '1px solid #e0d3b0', borderRadius: 9, padding: '0.5rem 0.7rem', background: 'var(--bp-panel)' }}>
              <strong style={{ fontSize: 12.5, color: 'var(--bp-text)' }}>💸 Costos/mes (líneas: {rep.costosMes.lineas})</strong>
            ...
            <div style={{ border: '1px solid #e0d3b0', borderRadius: 9, padding: '0.5rem 0.7rem', background: 'var(--bp-panel)' }}>
              <strong style={{ fontSize: 12.5, color: 'var(--bp-text)' }}>👥 Nómina</strong>
              <div style={{ fontSize: 12, color: 'var(--bp-muted)', marginTop: 3 }}>{rep.nomina.personasInternas} personas · {rep.nomina.personasSinCifra ? `${rep.nomina.personasSinCifra} sin cifra` : `$${rep.nomina.mensualConocido}/mes`}</div>
            </div>
          </div>
          ...
          <div style={{ border: `1px solid ${rep.hallazgos.length ? '#f0c9c2' : '#bfe6cf'}`, borderRadius: 9, padding: '0.6rem 0.8rem', background: rep.hallazgos.length ? '#fdf3f1' : '#eefaf2' }}>
            ...
            {rep.hallazgos.length > 0 && <ul style={{ margin: '0.3rem 0 0', paddingLeft: '1.1rem', fontSize: 12.5, color: 'var(--bp-text)' }}>{rep.hallazgos.map((h, i) => <li key={i} style={{ marginBottom: 2 }}>{h}</li>)}</ul>}
          </div>
        </>
      )}
    </section>
...
function PanelEntregables({ proyectoId, onVolver }: { proyectoId: string; onVolver: () => void }) {
  ...
  return (
    <section>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
        <h2 style={{ margin: 0 }}>📦 Generación de entregables <span style={{ fontSize: 13, color: 'var(--bp-muted)' }}>· cada paquete es un libro con índice</span></h2>
        <button style={btn} onClick={onVolver}>← Planos</button>
      </div>
