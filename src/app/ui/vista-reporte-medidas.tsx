...
const card: CSSProperties = { border: '1px solid #dde', borderRadius: 10, padding: '0.6rem 0.8rem', background: 'var(--bp-panel)', minWidth: 110 };
const num: CSSProperties = { fontSize: 20, fontWeight: 'bold', color: 'var(--bp-gold)' };
const cap: CSSProperties = { fontSize: 11, color: 'var(--bp-muted)', textTransform: 'uppercase', letterSpacing: 0.3 };
const th: CSSProperties = { textAlign: 'left', borderBottom: '2px solid #dde', padding: '4px 8px', fontSize: 12, color: 'var(--bp-muted)' };
  ...
  return (
    <div className="reporte-medidas">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
        <h2 style={{ margin: 0 }}>📐 Reporte de medidas <span style={{ fontSize: 13, color: 'var(--bp-muted)' }}>· {sedeNombre}</span></h2>
        <div className="no-print" style={{ display: 'flex', gap: '0.4rem' }}>
          <button style={{ padding: '0.35rem 0.8rem', borderRadius: 6, border: '1px solid #999', background: 'var(--bp-panel)', cursor: 'pointer', fontSize: 13 }} onClick={() => window.print()}>🖨 Imprimir / PDF</button>
          {onCerrar && <button style={{ padding: '0.35rem 0.8rem', borderRadius: 6, border: '1px solid #999', background: 'var(--bp-panel)', cursor: 'pointer', fontSize: 13 }} onClick={onCerrar}>← Volver</button>}
        </div>
      </div>
      <p style={{ fontSize: 12, color: 'var(--bp-muted)', margin: '0.2rem 0 0.8rem' }}>Medidas reales del plano (mismo entregable que un escaneo LiDAR). El escaneo se hace en el teléfono (MAKE.PLAN / Polycam / Scaniverse) y se sube como .glb; aquí se reporta lo medido.</p>
      ...
      {r.nCuartos === 0 && <p style={{ color: 'var(--bp-muted)', fontSize: 13 }}>Aún no hay cuartos/áreas dibujados en esta sede. Créalos en el editor 2D o sube un escaneo.</p>}
      ...
      {capas.map((capa) => {
        ...
        return (
          <div key={capa} style={{ marginBottom: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', color: 'var(--bp-text)', borderBottom: '2px solid #33415c', paddingBottom: 3, marginBottom: 4 }}>
                ...
                <tbody>
                  {cuartos.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td style={{ ...td, color: 'var(--bp-muted)' }}>{c.tipo === 'habitacion' ? 'Habitación' : 'Área'}</td>
                      ...
                      <td style={{ ...td, textAlign: 'right', color: 'var(--bp-muted)', fontVariantNumeric: 'tabular-nums' }}>{metros(c.perimetro)}</td>
