...
export async function actualizarSede(id: string, patch: Partial<Sede>): Promise<void> {
  ...
  for (const k of ['direccion', 'lat', 'lng', 'medidas', 'rentaMensual', 'footAncho', 'footAlto', 'poligono', 'muroExterior', 'muroInterior', 'acabadoPiso', 'acabadoMuros', 'existe'] as const) if (patch[k] !== undefined) merged[k] = patch[k];
