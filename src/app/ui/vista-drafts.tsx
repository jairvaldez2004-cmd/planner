    ...
    <section>
      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
        <input
          ...
          style={{ padding: '0.35rem 0.6rem', borderRadius: 6, border: '1px solid var(--bp-border)', flex: 1, minWidth: 180 }}
        />
      ...
      {filtrados.length === 0 && !loading && (
        <p style={{ color: 'var(--bp-muted)' }}>No hay drafts{busqueda ? ' que coincidan.' : '. Crea uno en "Nuevo Plano".'}</p>
      )}
      {filtrados.map((p) => {
        ...
        return (
          <div key={p.id} style={card}>
            ...
            <div style={row}>
              <button style={{ ...btn, background: 'var(--bp-panel-alt)' }} onClick={() => onAbrir(p.id)}>Abrir</button>
