  ...
  return (
    <div className="instructivo-raiz">
      ...
      <header style={{ borderBottom: '2px solid #33415c', paddingBottom: '0.5rem', marginBottom: '1rem' }}>
        ...
        {info && <p style={{ margin: '0.25rem 0 0', fontSize: 13, color: 'var(--bp-muted)' }}><strong>Etapa {info.n} · {info.label}</strong> — {info.descripcion}</p>}
        <p style={{ margin: '0.2rem 0 0', fontSize: 11.5, color: 'var(--bp-muted)' }}>
      ...
      {total === 0 && <p style={{ color: 'var(--bp-muted)' }}>No hay procesos vigentes en esta etapa todavía.</p>}
      ...
      {fases.map((f) => {
        ...
        return (
          <section key={f.id} className="fase-bloque" style={{ marginBottom: '1.4rem' }}>
            <h2 style={{ fontSize: 16, background: 'var(--bp-panel-alt)', padding: '0.35rem 0.6rem', borderRadius: 6, margin: '0 0 0.6rem' }}>
              {f.label} <span style={{ fontWeight: 'normal', color: 'var(--bp-muted)', fontSize: 13 }}>({lista.length} procesos)</span>
            </h2>
            ...
            {lista.map((p) => {
              ...
              return (
                <article key={p.id} className="proceso-ficha" style={{ border: '1px solid #d5d9e2', borderRadius: 8, padding: '0.6rem 0.75rem', marginBottom: '0.6rem', background: 'var(--bp-panel)' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: 11.5, color: 'var(--bp-muted)' }}>· {d?.nombre ?? '—'}</span>
                    {!naceEn(p, etapa) && <span style={{ fontSize: 10.5, color: 'var(--bp-muted)' }}>(desde la etapa {nEtapa(p.etapaDesde)})</span>}
                    {p.etapaHasta === etapa && <span style={{ fontSize: 10.5, color: '#c0392b', fontWeight: 'bold' }}>· último uso en esta etapa</span>}
                  </div>

                  {p.descripcion && <p style={{ margin: '0.3rem 0 0', fontSize: 12.5, color: 'var(--bp-muted)' }}>{p.descripcion}</p>}
                  ...
                  {pasos.length > 0 && (
                    <div style={{ marginTop: '0.4rem' }}>
                      <div style={{ fontSize: 11, fontWeight: 'bold', color: 'var(--bp-muted)', textTransform: 'uppercase', letterSpacing: 0.3 }}>Paso a paso</div>
                  ...
                  {p.manuales && Object.values(p.manuales).some(Boolean) && (
                    <div style={{ marginTop: '0.4rem' }}>
                      <div style={{ fontSize: 11, fontWeight: 'bold', color: 'var(--bp-muted)', textTransform: 'uppercase', letterSpacing: 0.3 }}>Manuales de herramientas / equipo</div>
                  ...
                  {p.apoyos && p.apoyos.length > 0 && (
                    <div style={{ marginTop: '0.4rem' }}>
                      <div style={{ fontSize: 11, fontWeight: 'bold', color: 'var(--bp-muted)', textTransform: 'uppercase', letterSpacing: 0.3 }}>🎥 Videos y documentos de apoyo</div>
                      {p.apoyos.map((a) => (
                        <div key={a.id} style={{ fontSize: 12.5, marginTop: 2 }}>
                          {a.tipo === 'video' ? '🎥' : a.tipo === 'documento' ? '📄' : '🔗'}{' '}
                          <a href={a.url} target="_blank" rel="noreferrer" style={{ color: 'var(--bp-gold)' }}>{a.titulo || a.url}</a>
                          {a.nota ? <span style={{ color: 'var(--bp-muted)' }}> — {a.nota}</span> : null}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ marginTop: '0.45rem', borderTop: '1px dashed #dfe3ea', paddingTop: '0.35rem' }}>
                    <div style={{ fontSize: 11, fontWeight: 'bold', color: 'var(--bp-muted)', textTransform: 'uppercase', letterSpacing: 0.3 }}>Qué sigue</div>
                    {p.ramas.filter((r) => r.destinoProcesoId).length === 0 && (
                      <p style={{ margin: '0.15rem 0 0', fontSize: 12.5, color: 'var(--bp-muted)' }}>Aquí termina este camino.</p>
                    )}
                    {p.ramas.filter((r) => r.destinoProcesoId).map((r) => {
                      ...
                      return (
                        <p key={r.id} style={{ margin: '0.15rem 0 0', fontSize: 12.5 }}>
                          ...
                          {' → '}
                          {futuro
                            ? <span style={{ color: 'var(--bp-muted)' }}>se guarda para <strong>{dest.nombre}</strong>, que arranca en la etapa {nEtapa(dest.etapaDesde)} ({etapaInfo(dest.etapaDesde)?.label}).</span>
                            : <>paso <strong>{numeracion.get(dest.id)}</strong> · {dest.nombre} <span style={{ color: 'var(--bp-muted)' }}>({deptoDe(dest.departamentoId)?.nombre ?? '—'})</span></>}
                        </p>
                      );
                    })}
                  </div>
      ...
      <footer style={{ borderTop: '1px solid var(--bp-border)', marginTop: '1rem', paddingTop: '0.4rem', fontSize: 10.5, color: 'var(--bp-muted)' }}>
...
function Fila({ etiqueta, valor }: { etiqueta: string; valor: string }) {
  ...
  return (
    <tr>
      <td style={{ padding: '0.12rem 0.5rem 0.12rem 0', color: 'var(--bp-muted)', whiteSpace: 'nowrap', verticalAlign: 'top', width: 110 }}>{etiqueta}</td>
